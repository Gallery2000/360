...
var prizeArr = null;
$(function() {
  var danindex = 0; //砸我运动的蛋索引
  var dansuiSwitch = true; //碎蛋开关
  var suiArr = null; //被打开的蛋
  var count = 8; //可用砸蛋的次数
  FZ(20, 375); //REM适配
  //滚球
  ...
  setTimeout(function() {
    //
    suiArr = !local.fetch("sui") ? [] : local.fetch("sui");
    ...
    //当前剩余可玩次数
    $("#countZa").text(count - suiArr.length);
    //中奖列表
    prizeList();
  });
  ...
  $("#danlist .full").on("click", function() {
    ...
    $(".chuizi").animate(
      { top: eggOffset.top - 30, right: left - 30 },
      1200,
      function() {
        obj.addClass("eggbreak"); //点击之后添加被杂碎的动画
        setTimeout(function() {
  ...
  function jindanAnimation(aniClass, son, time) {
    var eggint = setInterval(function() {
      if (dansuiSwitch == true) {
        danindex =
          danindex == $("#danlist").find(".full").length ? 0 : danindex;
        $("#danlist")
          .find(".full")
          .eq(danindex)
          .addClass(aniClass)
          .siblings(son)
          .removeClass(aniClass);
        danindex++;
      } else {
        $("#danlist")
          .find(".full")
...
function alertCommon(imageUrl, advIntro) {
  var strs =
    ...
    imageUrl +
    '"></div><div class="detail"><div class="topic">' +
    advIntro +
  ...
  setTimeout(function() {
    $("#dialog5").append(
      '<span id="close" class="close-btn closetc iconfont"></span>'
    );
  }, 1500);
}
function boxHandle(){
