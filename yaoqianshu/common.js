...
$(function() {
	...
	if ($('#countZa').text() == '8') {
		...
		$('.bgColor').css('width', local.fetch('width') + 'rem')
	}

	//超过0点清空记录
	var agoTime = local.fetch('agoTime') ? local.fetch('agoTime') : new Date().getTime();
	if (new Date().getTime() > agoTime) {
		local.save('agoTime'),"";
		local.save('agoTime', new Date(new Date().toLocaleDateString()).getTime() / 1 + 86400000);
		local.save('width',"")
		local.save('index',"")
		local.save('index1',"")
		local.save('index2',"")
	...
	if (local.fetch('index')) {
		$('.money').hide()
	}
	if (local.fetch('index1')) {
		$('.money').hide()
		$('.money2').hide()
	}
	if (local.fetch('index2')) {
		$('.liping').hide()
		$('.money').hide()
		$('.money2').hide()
	}
...
// 	}, 3000)
	...
	function myMove() {
		...
		$('.yaoqianshu').addClass('yaoshuoGif').css('background-image', 'url(' + img_bao + ')')
		$('.liuzhi1,.liuzhi2,.liuzhi3,.liuzhi4,.liuzhi5').addClass('liuzhiDong')
		setTimeout(function() {

			// debugger
			$('.yaoqianshu').removeClass('yaoshuoGif').css('background-image',
				'url(https://hdggcdn.bayimob.com/hdggstatic/yaoshu1/shu.png)');
			// upCount();
			setp(count)
	...
	function setp(n) {
		var width = local.fetch('width') ? local.fetch('width') : 0
		...
		if (n == 7) {
			...
			local.save('width', Number(width) + 1.5)
		}
		if (n == 6) {
			$('.bgColor').animate({
				'width': Number(width) + 2 + 'rem'
			}, 400)
			local.save('width', Number(width) + 2)
			setTimeout(function() {
				pabulic(n)
			}, 410)
		}
		if (n == 5) {
			...
			local.save('width', Number(width) + 2.3)
			setTimeout(function() {
				pabulic(n)
			}, 410)
		}
		if (n == 4) {
			...
			local.save('width', Number(width) + 2.7)
			setTimeout(function() {
				pabulic(n)
			}, 410)
		}
		if (n == 3) {
			...
			local.save('width', Number(width) + 1)
		}
...
function alertCommon(imageUrl, advIntro) {
	var strs =
		'<div class="popShowPrize" id="dialog5" style="display: block; transform-origin: 0px 0px 0px; opacity: 1; transform: scale(1, 1);"><div class="titles"></div><div class="leftHua"></div><div class="rightHua"></div><div class="caiguang"></div><div class="showPrize-dialog modal-body"><div class="red-bg"><div class="card-bg""><img class="resAd" src="' +
...
$(document).on("click", ".m-close", function() {
  ...
  function prizeList() {
	...
	$("#countZa").text(8-prizeArr.length);
	if(!prizeArr.length){
		redpack();
	}
	var strs = "";
